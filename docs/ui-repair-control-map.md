...
| OpenRouter key | Store BYOK key for the current app-window session | Provider status near key field | Survives refresh; never commit to project files. |
| Test OpenRouter | Validate session key | Provider status in Settings | Freeform replies should only be live after this passes. |
