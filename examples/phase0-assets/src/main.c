...
int main(bool hardReset)
{
    ...
    PAL_setPalette(PAL1, phase0_player.palette->data, DMA);
    VDP_setTextPalette(PAL1);
    VDP_drawText("Drive16 Phase 0", 9, 3);
    VDP_drawText("D-pad moves sprite", 7, 5);
    VDP_drawText("Bundled VGM loop plays", 5, 24);

    player = SPR_addSprite(&phase0_player, playerX, playerY, TILE_ATTR(PAL1, TRUE, FALSE, FALSE));
    if (player == NULL)
        SYS_die("Sprite allocation failed");

    XGM_startPlay(phase0_loop);
