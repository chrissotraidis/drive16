...
const PASS_PROMPTS = {
  ...
  art: ({ projectPath, request }) => [
    ...
    "- For each role: call the SYNCHRONOUS drive16-comfyui generate_sprite (one call returns the finished sprite) with a role-appropriate prompt, a symbol that names the role, and the role field set. Do NOT use enqueue_sprite/job_status here — polling wastes your step budget. Copy each SGDK-ready PNG into res/, add its SPRITE line to res/resources.res (path relative to res/), declare it in res/resources.h, and wire it in src/main.c with the sprite engine (SPR_init once, SPR_addSprite, SPR_setPosition, SPR_update in the loop).",
    "- Exact sprite-engine API (do not use anything else): SPR_init(); store each sprite as a POINTER, Sprite *s = SPR_addSprite(&resource, x, y, TILE_ATTR(PAL2, TRUE, FALSE, FALSE)); SPR_setPosition(s, x, y); SPR_setVisibility(s, VISIBLE) or SPR_setVisibility(s, HIDDEN); SPR_update() every frame before SYS_doVBlankProcess(). Never declare Sprite instances as structs, never use V_ON/V_OFF, no priority field, no xExtent/yExtent.",
    "- Wire sprites for at most 2 roles before your first rebuild; a third role is optional polish only if steps remain after the build verifies.",
  ...
  music: ({ projectPath, request }) => [
    ...
    "- Read the matching template in assets/enhancements/mml/genre-templates/ first (action-tension, bouncy-arcade, puzzle-groove, adventure-lite) — each is a verified-good, quality-passing arrangement. Start from the closest one: keep its channel architecture and instrument blocks, replace melodies/tempo to fit this game. Put the channel letter at the start of every line.",
    "- Consult corpus/mml/ctrmml-megadrive.md for syntax questions; copy instrument blocks from the template you start from.",
