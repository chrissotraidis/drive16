...
const PASS_PROMPTS = {
  ...
  art: ({ projectPath, request }) => [
    ...
    "- For each role: drive16-comfyui generate_sprite with a role-appropriate prompt, a symbol that names the role, and the role field set to the gameplay role. Copy each SGDK-ready PNG into res/, add its SPRITE line to res/resources.res (path relative to res/), declare it in res/resources.h, and wire it in src/main.c with the sprite engine (SPR_init once, SPR_addSprite, SPR_setPosition, SPR_update in the loop).",
    "- Optionally give plane B a generated backdrop: enqueue_background (prompt, symbol, role), poll job_status, fetch_result, then wire the IMAGE resource with VDP_drawImageEx only if it fits the scene.",
